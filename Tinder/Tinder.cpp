...
Tinder::Tinder(string hair, string name, string surname, int age,
	string zodiac_sign, string town, string hair_f, string zodiac_sign_f,
...
void Tinder::Run(Tinder tn, vector<Tinder> vec)
{
	...
	{
		vec[i].count = 0;
		if (tn.hair_f == vec[i].hair && vec[i].hair_f == tn.hair)
			vec[i].count++;
...
void Tinder::Print()
{
	cout << name << " " << surname << " " << age << " года (лет) " << zodiac_sign
		<< " родной город - " << town << endl << "Интересы:";
	interest(); 
	cout << "\nИщет пару со следующими данными:\n цвет волос - " << hair_f << " знак зодиака - " << zodiac_sign_f
		<< " из города " << town_f << " " << "в возрасте от" << " " << age_f
		<< " до" << " " << age_f2 << " лет\n"  << endl;
